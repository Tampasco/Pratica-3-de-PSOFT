...
public class ProdutoAlterarServiceTests {
    ...
    void quandoNovoNomeValido() {
        ...
        @Test
        @DisplayName("Produto Desatualizado armazenado de forma errada")
        void produtoDesatualizado() {
                //Arrange
                produto.setNome("Produto Dez Atualizado");
                Mockito.when(produtoRepository.update(produto))
                        .thenReturn(Produto.builder()
                ...
                //Assert
                assertEquals("Produto Dez Atualizado", resultado.getNome());
                assertNotEquals("Produto Dez", produtoRepository.find(10L).getNome());
        }

        @Test
        @DisplayName("Produtos repetidos ao realizar uma alteração")
        void produtosRepetidos() {
                    //Arrange
                    produto.setNome("Produto Dez Atualizado");
                    Mockito.when(produtoRepository.update(produto))
                            .thenReturn(Produto.builder()
                    ...
                    assertEquals("Produto Dez Atualizado", resultado.getNome());
                    assertEquals(produtoRepository.findAll().size(), 1);
        }

        @Test
        @DisplayName("Preço Invalido abaixo de zero")
        void precoInvalido() {
                    //Arrange
                    produto.setNome("Produto Dez Atualizado");
                    Mockito.when(produtoRepository.update(produto))
                    ...
                    Produto resultado = driver.alterar(produto);
                    //Assert
                    assertNotEquals("Produto Dez Atualizado", resultado.getNome());
                    assertNotEquals(produtoRepository.find(10L).getPreco(), -25.00);
